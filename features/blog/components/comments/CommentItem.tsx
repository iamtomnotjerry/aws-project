...
export const CommentItem = ({ comment, onReply, isReply = false }: CommentItemProps) => {
  ...
  return (
    ...
    >
      {/* Thread Line */}
      {isReply && (
        <div className="absolute -left-6 lg:-left-10 top-0 bottom-0 flex justify-center">
          <div className="w-px h-full bg-white/5 group-hover:bg-primary/20 transition-colors" />
        </div>
      )}
      ...
      <div className={`p-6 rounded-2xl border transition-all duration-300 ${
        ...
        <div className="flex items-center justify-between gap-4 mb-4">
          ...
          {!isReply && onReply && (
            <button
              onClick={() => onReply(comment.id, comment.user.name || "Bao's Reader")}
              className="text-[9px] font-bold uppercase tracking-widest text-slate-600 hover:text-primary transition-colors flex items-center gap-1.5"
            >
              <Reply size={12} />
              Reply
            </button>
